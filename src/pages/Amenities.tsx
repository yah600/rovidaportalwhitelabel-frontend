...
const Amenities = () => {
  ...
  const breadcrumbItems = [
    { label: t('home'), href: '/' },
    { label: t('amenity management'), href: '/amenities' },
  ];
  ...
  return (
    <div className="flex flex-1 flex-col gap-4">
      ...
      <header className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold md:text-3xl text-page-title">{t('amenity management')}</h1>
        <Button className="btn-primary">
          <PlusCircle className="mr-2 h-4 w-4" /> {t('add new amenity')}
        </Button>
      </header>
      <p className="text-rovida-slate-green-gray">{t('manage book amenities')}</p>
      ...
      {hasAmenities ? (
        ...
        <Card className="flex flex-1 items-center justify-center rounded-lg border border-dashed shadow-sm card-rovida mt-4 p-8">
          <div className="flex flex-col items-center gap-2 text-rovida-slate-green-gray">
            ...
            <p>{t('amenities managed here')}</p>
            <Button variant="outline" className="mt-4 btn-secondary">
              <PlusCircle className="mr-2 h-4 w-4" /> {t('add first amenity')}
            </Button>
          </div>
        </Card>
      )}
