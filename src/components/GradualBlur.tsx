...
const mergeConfigs = (...configs: object[]) => configs.reduce((acc, c) => ({ ...acc, ...c }), {});
const getGradientDirection = (position: GradualBlurProps['position']) =>
  ({
    top: 'to top',
    bottom: 'to bottom',
    left: 'to left',
...
function GradualBlur(props: GradualBlurProps) {
  ...
  const blurDivs = useMemo(() => {
    ...
    const currentStrength =
      isHovered && config.hoverIntensity ? (config.strength || DEFAULT_CONFIG.strength) * config.hoverIntensity : (config.strength || DEFAULT_CONFIG.strength);
  ...
  const containerStyle = useMemo(() => {
    ...
    const baseStyle: React.CSSProperties = {
      ...
      zIndex: isPageTarget ? (config.zIndex || DEFAULT_CONFIG.zIndex) + 100 : config.zIndex,
      ...config.style
    };
  ...
  useEffect(() => {
    if (isVisible && animated === 'scroll' && onAnimationComplete) {
      const ms = parseFloat(duration || DEFAULT_CONFIG.duration) * 1000;
